...
parser.add_argument('--factor', default=0.3, help='Market order factor', type=float)
...
def action_wrapper(order_direction, factor):
	 def action(remaining_time, amount, lob_copy):
	 	if remaining_time == args.H:
	 		...
	 		if order_direction == 1:
	 			return int(current_mid_price * (1 + factor))
	 		else:
	 			return int(current_mid_price * (1 - factor))
	 	else:
...
if args.mode == 'train':
	action_func = action_wrapper(args.order_direction, args.factor)
	rewards = evaluate_policy(args.train_start, args.train_end, args.order_direction, args.V, args.H, args.T, oq, mq, action_func)
	print(rewards)

...
class Limit_Order_book(object):
    ...
    def update_own_order(self, price, amount = None):
        ...
        if price != self.own_price: #Only need to update if different price
            ...
            self.add_order(amount, self.own_price, self.own_trade_type, own=True)
    ...
    def partial_execution(self, size, price, direction, own=False):
        ...
        if own:
            # If my own order get executed, get reward
            ...
            # Other peopler's order get executed, but there are our own orders
